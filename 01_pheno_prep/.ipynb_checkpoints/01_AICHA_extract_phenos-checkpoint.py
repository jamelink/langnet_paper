...
#load QC'ed subs
s_list = open("/data/workspaces/lag/workspaces/lg-ukbiobank/projects/multilateral/HICA/pheno_mpi/subjlist_N30660.txt")
...
#SENT CORE
#nodes
df_sent_nodes = pd.DataFrame(index=subj_list, columns=sent_node_names_l+sent_node_names_r)
df_sent_nodes_bi = pd.DataFrame(index=subj_list, columns=sent_node_names_l+sent_node_names_r)
df_sent_nodes_asym = pd.DataFrame(index=subj_list, columns=sent_node_names_asym)
df_sent_nodes_asym_bi = pd.DataFrame(index=subj_list, columns=sent_node_names_asym)
#edges
...
for sub in subj_list: 
    ...
    ## CALCULATE NODE DEGREE CONNECTIVITY 
        #SENT CORE
    #within hemi only
    sent_nodes_l = np.sum(l_sent, axis=1)
    sent_nodes_r = np.sum(r_sent, axis=1)
    #whole-brain
    #sent_nodes_bi = np.sum(bi_sent, axis=1)
    #sent_nodes_bi_l = sent_nodes_bi[np.ix_(range(0,len(sent_core_bi_ind), 2))]
    #sent_nodes_bi_r = sent_nodes_bi[np.ix_(range(1,len(sent_core_bi_ind)+1, 2))]
    ...
    #set edges where both correlations are set to zero to nan (by first concatenating in 3rd dimension and then doing all over 3rd dimension).
    #sent_edges_asym[np.dstack((l_sent == 0, r_sent == 0)).all(axis=2)] = np.nan

    ## NODE ASYMMETRIES
    #For nodes we use the asymmetry index ((2*(L-R))/(L+R))
        #SENT CORE
    sent_nodes_asym = np.divide((np.subtract(sent_nodes_l, sent_nodes_r)), np.add(sent_nodes_l, sent_nodes_r))
    #sent_nodes_asym_bi = np.divide((np.subtract(sent_nodes_bi_l, sent_nodes_bi_r)), np.add(sent_nodes_bi_l, sent_nodes_bi_r))
    ...
    np.savetxt(os.path.join(base_path, "pheno", "asym_mats_sent", "{0}_sent_hdmat.txt".format(sub)), sent_edges_asym)
        ...
        #SENT CORE
        #nodes
    df_sent_nodes.loc[sub] = np.concatenate((sent_nodes_l, sent_nodes_r))
    #df_sent_nodes_bi.loc[sub] = np.concatenate((sent_nodes_bi_l, sent_nodes_bi_r))
    df_sent_nodes_asym.loc[sub] = sent_nodes_asym
    #df_sent_nodes_asym_bi.loc[sub] = sent_nodes_asym_bi
        #edges
...
#SAVE ALL DFS
#nodes
df_sent_nodes.to_csv(os.path.join(base_path, "pheno", "sent_nodes_N{0}.csv".format(subs_no))) 
#df_sent_nodes_bi.to_csv(os.path.join(base_path, "pheno", "sent_nodes_bi_N{0}.csv".format(subs_no)))
df_sent_nodes_asym.to_csv(os.path.join(base_path, "pheno", "sent_nodes_asym_N{0}.csv".format(subs_no)))
#df_sent_nodes_asym_bi.to_csv(os.path.join(base_path, "pheno", "sent_nodes_bi_asym_N{0}.csv".format(subs_no)))
